import { useDPR, type UseDPRInstance } from './useDPR.ts';
...
export function useScreen(options: UseScreenOptions = {}): UseScreenInstance {
    ...
    let dprWatcher: UseDPRInstance | null = null;
    ...
    const api = {
        ...
        get dpr() {
            return dprWatcher?.value ?? (window.devicePixelRatio || 1);
        },
        start,
        stop
    };

    dprWatcher = useDPR(callCallbacks);
